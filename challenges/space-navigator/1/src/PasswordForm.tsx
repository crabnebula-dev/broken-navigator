...
const submit = async () => {
  let result = await invoke("check_password", { password: input() });
  if (typeof result === "boolean") {
    ...
    console.error(
      "💩",
      "failed to get password result from backend, this isn't an intentional error",
    );
  }
};

const PasswordForm: VoidComponent = () => {
  return (
    <form
      onSubmit={async (e) => {
        e.preventDefault();
